...
async function main() {
  // Safe to run on every deploy: detect "already seeded" by ROOMS (only the
  // seed creates rooms — a stray signup won't block it). FORCE_SEED=1 re-seeds.
  const existingRooms = await prisma.room.count()
  if (existingRooms > 0 && process.env.FORCE_SEED !== '1') {
    console.log(`✅ Database already seeded (${existingRooms} rooms) — skipping. Set FORCE_SEED=1 to re-seed.`)
    return
  }
